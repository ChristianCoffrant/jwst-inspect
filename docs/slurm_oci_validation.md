...
As of 2026-07-01, native Slurm OCI is enabled on the workstation. The site
...
```text
Commit: dbfc31fce2bf04f35702b1bb3c3b9b1fd77bb7aa
Slurm jobs: 262,263,264,265,266,267
Run dir: /data/groups/autonomous/runs/20260701T011642Z-jwst-e2e
Artifact manifest: /data/groups/autonomous/runs/20260701T011642Z-jwst-e2e/artifact_manifest.json
```
...
Manifest checks from that run:

```text
artifact_manifest.json: status=passed, errors=[]
replicator/dataset_manifest.json: status=passed, isaac.available=true, frame_count=2
isaaclab/episode_log.json: status=passed, isaaclab.available=true, episode_count=1, steps=32
evaluation/r2p_smoke_summary.json: status=passed
```
...
An additional runtime-info GPU probe submitted on 2026-07-01 as job `269`
remained pending on resources and was cancelled without running. The GPU-backed
Replicator, Isaac Lab, and R2P jobs above completed successfully in the E2E
chain.
