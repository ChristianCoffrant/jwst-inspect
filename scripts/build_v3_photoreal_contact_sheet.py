...
OUT = ROOT / "outputs" / "v3_photoreal"
MANIFEST = OUT / "v3_photoreal_manifest.json"
...
def main() -> int:
    ...
    cell_w, image_h, text_h = 420, 236, 96
    ...
    for idx, loop in enumerate(loops):
        ...
        if not image_path.exists():
            parts = image_path.as_posix().split("/outputs/v3_photoreal/", 1)
            if len(parts) == 2:
        ...
        if line:
            lines.append(line)
        draw.text((x + 14, y + image_h + 40), "\n".join(lines[:3]), fill=(174, 202, 218), font=body_font)
    out = OUT / "v3_photoreal_contact_sheet.png"
    sheet.save(out)
